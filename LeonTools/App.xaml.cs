...
namespace LeonTools
{
    ...
    public partial class App
    {
        ...
        {
            string mutexName = "55B5D282-DCB3-4C05-91D2-408E7CA72058";
            ...
            if (!createNew)
            {
                try
                {
                    var processes = Process.GetProcessesByName("LeonTools.exe");
                    if (!processes.Any())
                    {
                        MessageBox.Show("程序已启动，无法启动多个进程！", "错误提示", MessageBoxButton.OK, MessageBoxImage.Error);
                    }
                    else
                    {
